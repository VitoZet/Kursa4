...
class Gena(QMainWindow):
    ...
    def def_yd_descript1_on(self): #Включаем возможность ввода текста в расширенный заголовок, когда известна длина основного загоовка
        len_title = len(self.yd_title.text())
        if len_title > 0:
    ...
    def def_yd_descript2_on(self, text): # Включаем возможность ввода текста, когда известна, длина расширенного заголовка.
        len_descript1 = len(self.yd_descript1.text())
        if len_descript1 > 0:
            self.yd_descript2.setEnabled(True)
        else:
            self.yd_descript2.setEnabled(False)
        self.view_yd_descript1.setText(' - ' + text) # выводим на визуализацию
        self.view_yd_descript1.adjustSize()
    ...
    def def_adw_len_title1(self, text): # Вычисляем и показываем заголовок в ADWORDS
        ...
        self.view_adw_title1.setText(text)
        self.view_adw_title1.adjustSize()
    ...
    def def_adw_len_title2(self, text): #Вычисляем и показываем расширенный заголовок в ADWORDS
        ...
        self.view_adw_title2.setText(' - ' + text)
        self.view_adw_title2.adjustSize()
